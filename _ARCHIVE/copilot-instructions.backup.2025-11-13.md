...
Use american-english spelling for all artifacts, not british-english (e.g. "organization" not "organisation").
...
- Examples placement: Put `@example` blocks on the symbol’s top-level docblock (exported interfaces/types/classes), not on members. Keep member docs concise (what the field means), and centralize usage examples on the parent symbol.
