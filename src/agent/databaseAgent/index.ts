...
export class DatabaseAgent extends BaseAgentConfig {
  ...
  /**
   * Executes a generic query and returns results wrapped in AgentResponse<T> for consistent formatting.
   ...
   * @returns {Promise<import("@agent/communicationAgent").AgentResponse<CategoryRecord[]>>} Structured response containing query results.
   */
  async executeQueryResponse(
    categoryId: CategoryId,
    criteria: Record<string, unknown> = {},
    options: QueryOptions = {}
  ): Promise<
    import("@agent/communicationAgent").AgentResponse<CategoryRecord[]>
  > {
    ...
    try {
      ...
      const results = await this.executeQuery(categoryId, criteria, options);
      const duration = Date.now() - startTime;

      return createSuccessResponse(results, {
        message: `Query executed successfully on ${categoryId}`,
        metadata: {
          agentId: this.profile.id,
          operation: "executeQuery",
          duration,
          count: results.length,
          entityType: "CategoryRecord[]",
        ...
        errors: [
          {
            ...
            severity: isNotFound ? "medium" : "high",
            code: isNotFound ? "DATA_SOURCE_NOT_FOUND" : "QUERY_EXECUTION_FAILED",
            suggestions: isNotFound
              ? [
                  `Verify category ID "${categoryId}" exists`,
                  "Check available categories with getAvailableCategories()",
                ]
              : [
                  "Verify query criteria format",
