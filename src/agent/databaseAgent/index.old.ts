...
// Define data types without importing from other agents
 ...
 * @example
 * ```ts
 * const database = new DatabaseAgent(dataSources);
...
export class DatabaseAgent {
   ...
   *
   * @param dataSources - Available data sources for querying.
   * @param [cacheDirPromise] - Optional override for the cache directory resolution.
   */
  constructor(dataSources: DataSource[], cacheDirPromise?: Promise<string>) {
    this.config = new DatabaseAgentConfig();
    this.cacheDirPromise = cacheDirPromise ?? ensureCacheDirectory();

    // Convert data sources array to map for efficient lookup
    ...
    if (dataSources.length === 0) {
      console.warn("[database-agent] No data sources provided");
    }
  }
  ...
  /**
   * Execute a query against any category by identifier.
   *
   * @param categoryId - Category identifier to query.
  ...
  async queryCategory(
    ...
    return this.executeQuery(categoryId, criteria, options);
  }
  ...
  // Note: Saved query functionality moved to orchestrator
  // DatabaseAgent focuses purely on data querying
  ...
  private async executeQuery(
    ...
    return this.telemetry(
      ...
      async () => {
        ...
        // Simplified caching - just check if cache exists and is recent
        if (cached) {
          const cacheAge = Date.now() - new Date(cached.timestamp).getTime();
          const maxCacheAge =
            this.config.getCachingConfig()?.cacheTTL || 300000; // 5 min default
  ...
  private performFilter(
    ...
    if (!dataSource) {
      console.warn(
        `[database-agent] No data source found for category: ${categoryId}`
      );
      return [];
    }
    ...
    return records.filter((record: CategoryRecord) =>
 ...
 * ```
 */
export function createDatabaseAgent(dataSources: DataSource[]): DatabaseAgent {
  return new DatabaseAgent(dataSources);
}

// Export configuration types and instances for external use
export { DatabaseAgentConfig } from "./config";
export { databaseAgentConfig } from "./agent.config";
