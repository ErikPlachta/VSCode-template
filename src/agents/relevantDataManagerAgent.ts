...
export interface TestRunnerDefinition {
  /** Execution language or environment for the test. */
  language: string;
  /** The primary command or module to execute. */
  entrypoint: string;
  /** Optional command arguments. */
  args?: string[];
  /** Optional working directory relative to the repository root. */
  workingDirectory?: string;
  /** Optional environment variables to supply to the runner. */
  environment?: Record<string, string>;
}

export interface TestExpectation {
  /** Human friendly summary of the verification performed. */
  description: string;
  /** Guidance on what constitutes success for the expectation. */
...
export class RelevantDataManagerAgent {
  ...
  private loadPythonTypes(categoryDir: string): PythonTypeDefinition[] {
    ...
    return this.collectFiles(target, [".json"]).map((file) => {
      const typeFile = this.loadJsonFile<RawPythonTypeFile>(
        path.join(process.cwd(), file),
        `python types '${file}'`
      );
      if (!typeFile.name || !typeFile.schema) {
  ...
  private loadTests(categoryDir: string): CategoryTestArtefact[] {
    ...
    return this.collectFiles(target, [".json"]).map((file) => {
      ...
      if (!test.name || !test.runner || !test.runner.language || !test.runner.entrypoint) {
        throw new Error(`Test file '${file}' is missing required runner metadata.`);
      }
      if (test.runner.args && !Array.isArray(test.runner.args)) {
        throw new Error(`Test runner args in '${file}' must be an array when provided.`);
      }
      return test;
    });
  }
  ...
  private loadRecords(
    ...
    if (Array.isArray(raw)) {
      for (const entry of raw) {
        ...
        this.validateRecord(record, requirements, categoryId);
        records.push(record);
      }
      return records;
    }
    ...
    if (!raw || typeof raw !== "object") {
      throw new Error(`Records at '${recordsPath}' must be an array or object map.`);
    }

    for (const [recordId, entry] of Object.entries(raw)) {
      if (!entry || typeof entry !== "object") {
        throw new Error(`Record '${recordId}' in category '${categoryId}' is not a valid object.`);
      }
      const recordFromFile = entry as Record<string, unknown>;
      const declaredId = typeof recordFromFile.id === "string" ? recordFromFile.id : undefined;
      if (declaredId && declaredId !== recordId) {
        throw new Error(
          `Record '${recordId}' in category '${categoryId}' declares mismatched id '${declaredId}'.`
        );
      }
      const record = { id: recordId, ...recordFromFile } as CategoryRecord;
      this.validateRecord(record, requirements, categoryId);
      records.push(record);
    }
  ...
  private loadRelationships(categoryDir: string, categoryId: string): RelationshipLoadResult {
    ...
    const raw = this.loadJsonFile<unknown>(relationshipsPath, `relationships for ${categoryId}`);
    const entries: RawRelationshipEntry[] = [];
    ...
    if (Array.isArray(raw)) {
      ...
      for (const [key, value] of Object.entries(raw)) {
        if (!value || typeof value !== "object") {
          throw new Error(`Relationship '${key}' in '${relationshipsPath}' must be an object.`);
        }
        entries.push({ key, ...(value as Omit<RawRelationshipEntry, "key">) });
      }
    }

    if (entries.length === 0) {
      throw new Error(`Relationships at '${relationshipsPath}' must contain at least one entry.`);
    }
  ...
  private validateRecord(
    record: CategoryRecord,
    requirements: CategoryRequirements | undefined,
    categoryId: string
  ): void {
